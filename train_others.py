...
def test(g,adj, features, labels, idx_test, nclass, idx_train,
    ...
    if args.scaling_method == None:
        ...
        # print('acc_test,confidence': acc_test,(confidence.mean()))
        ...
        base_model  = get_models(args, nfeat, nclass, scaling_model=False)
        if args.scaling_method == 'TS':
            model = ModelWithTemperature(base_model, args.n_bins)
        elif args.scaling_method == 'MS':ece_criterion = _ECELoss(args.n_bins).cuda()
...
def temperature_scaling(g, adj, features, labels, nclass, idx_val, idx_train,
    ...
    state_dict = torch.load(model_b_scaling)
    base_model = get_models(args, features.shape[1], nclass, scaling_model=False)
...
def main(dataset):
    datastr = dataset
    splitstr = 'splits/'+dataset+'_split_0.6_0.2_'+'0'+'.npz'
